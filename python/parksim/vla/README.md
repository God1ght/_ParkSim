...
Long-horizon suites may set `PARKSIM_SUITE_SIMULATION_SPEEDUP` to decouple wall-clock scheduling from the simulator integration step. The formal protocol keeps `PARKSIM_SUITE_SIMULATION_STEP=0.1` for every method, records the speedup in every trace and run manifest, and rejects a run when `trace_sim_step_gap_count` is nonzero. A 1x-versus-accelerated deterministic equivalence smoke must pass before real-Qwen calibration; acceleration changes experiment turnaround only, not simulator time, vehicle dynamics step, Qwen decision epochs, or metric definitions.
