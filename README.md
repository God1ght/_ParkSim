...
For real Qwen runs, set `PARKSIM_SUITE_QWEN_MODE=real`; the suite starts one managed Qwen service, records `qwen_health.json` and `qwen_service.log`, and passes the managed endpoint to child benchmarks as `external` so the model is not reloaded for every condition. Set `PARKSIM_SUITE_QWEN_ENDPOINT` only when an external service is already running.
After report generation the suite runs `parksim.vla.paper_gate` and writes `reports/paper_gate/paper_gate.json` plus `paper_gate.md`. Use `PARKSIM_SUITE_GATE_PROFILE=pilot` for fast structural checks and `PARKSIM_SUITE_GATE_PROFILE=paper` for paper-scale coverage checks. Set `PARKSIM_SUITE_GATE_STRICT=1` when the suite should fail if the selected gate does not pass.
