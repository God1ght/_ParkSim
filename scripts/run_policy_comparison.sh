...
OUT_DIR="${PARKSIM_COMPARE_OUT_DIR:-$ROOT/experiments/qwen_vla_comparison/$(date +%Y%m%d_%H%M%S)}"
DURATION="${PARKSIM_COMPARE_DURATION:-90s}"
SPOT_INDEX="${PARKSIM_COMPARE_SPOT_INDEX:-7}"
SPAWN_TIME="${PARKSIM_COMPARE_SPAWN_TIME:-0.5}"
BACKGROUND_MODE="${PARKSIM_COMPARE_BACKGROUND_MODE:-rule_random}"
SPAWN_ENTERING="${PARKSIM_COMPARE_SPAWN_ENTERING:-0}"
SPAWN_EXITING="${PARKSIM_COMPARE_SPAWN_EXITING:-0}"
RANDOM_SEED="${PARKSIM_COMPARE_RANDOM_SEED:-0}"
QWEN_MODE="${PARKSIM_COMPARE_QWEN_MODE:-mock}"
QWEN_PORT="${PARKSIM_COMPARE_QWEN_PORT:-18087}"
QWEN_ENDPOINT="${PARKSIM_COMPARE_QWEN_ENDPOINT:-}"
QWEN_TIMEOUT="${PARKSIM_COMPARE_QWEN_TIMEOUT:-120.0}"
QWEN_STARTUP_TIMEOUT="${PARKSIM_COMPARE_QWEN_STARTUP_TIMEOUT:-900}"
...
start_qwen_if_needed() {
  ...
  case "$QWEN_MODE" in
    ...
    real)
      ...
      ;;
    external)
      echo "PARKSIM_COMPARE_QWEN_MODE=external requires PARKSIM_COMPARE_QWEN_ENDPOINT" >&2
      exit 1
      ;;
    *)
      echo "Unsupported PARKSIM_COMPARE_QWEN_MODE: $QWEN_MODE" >&2
      exit 1
      ;;
  esac
}
...
run_mode() {
  ...
  if [[ ! -f "$log_dir/vehicle_1_summary.json" ]]; then
    echo "$mode did not produce vehicle summary" >&2
    tail -n 160 "$sim_log" >&2 || true
    exit 1
  fi
}
...
echo "comparison_out_dir=$OUT_DIR"
