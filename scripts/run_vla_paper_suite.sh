...
AGENTS="${PARKSIM_SUITE_AGENTS:-rule_based greedy_nearest greedy_shortest_path risk_aware_rule bundle_risk_aware conflict_aware_bundle reservation_bundle rolling_horizon_bundle centralized_min_cost oracle_intent_bundle qwen_vla}"
...
SIMULATION_STEP="${PARKSIM_SUITE_SIMULATION_STEP:-0.1}"
SIMULATION_SPEEDUP="${PARKSIM_SUITE_SIMULATION_SPEEDUP:-1.0}"
WALL_TIMEOUT="${PARKSIM_SUITE_WALL_TIMEOUT:-}"
WALL_TIMEOUT_MULTIPLIER="${PARKSIM_SUITE_WALL_TIMEOUT_MULTIPLIER:-4}"
TRAFFIC_FLOW_MODE="${PARKSIM_SUITE_TRAFFIC_FLOW_MODE:-human_mixed_long_horizon}"
FLEET_CONTROL_MODE="${PARKSIM_SUITE_FLEET_CONTROL_MODE:-cloud_av}"
ENTRY_VEHICLE_AGENT_TYPE="${PARKSIM_SUITE_ENTRY_VEHICLE_AGENT_TYPE:-__agent__}"
EXIT_VEHICLE_AGENT_TYPE="${PARKSIM_SUITE_EXIT_VEHICLE_AGENT_TYPE:-__agent__}"
AV_SPAWN_ENTERING="${PARKSIM_SUITE_AV_SPAWN_ENTERING:-0}"
AV_SPAWN_EXITING="${PARKSIM_SUITE_AV_SPAWN_EXITING:-0}"
RESTORE_OBSTACLES_AS_EXIT_VEHICLES="${PARKSIM_SUITE_RESTORE_OBSTACLES_AS_EXIT_VEHICLES:-true}"
HUMAN_INTENT_HIDDEN_FRACTION="${PARKSIM_SUITE_HUMAN_INTENT_HIDDEN_FRACTION:-0.75}"
MAX_CONCURRENT_BACKGROUND_VEHICLES="${PARKSIM_SUITE_MAX_CONCURRENT_BACKGROUND_VEHICLES:-80}"
LONG_HORIZON_ENTER_INTERVAL_MEAN="${PARKSIM_SUITE_LONG_HORIZON_ENTER_INTERVAL_MEAN:-10.0}"
LONG_HORIZON_EXIT_INTERVAL_MEAN="${PARKSIM_SUITE_LONG_HORIZON_EXIT_INTERVAL_MEAN:-14.0}"
LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN="${PARKSIM_SUITE_LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN:-$LONG_HORIZON_ENTER_INTERVAL_MEAN}"
LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN="${PARKSIM_SUITE_LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN:-$LONG_HORIZON_EXIT_INTERVAL_MEAN}"
LONG_HORIZON_AV_ENTER_INTERVAL_MEAN="${PARKSIM_SUITE_LONG_HORIZON_AV_ENTER_INTERVAL_MEAN:-$LONG_HORIZON_ENTER_INTERVAL_MEAN}"
LONG_HORIZON_AV_EXIT_INTERVAL_MEAN="${PARKSIM_SUITE_LONG_HORIZON_AV_EXIT_INTERVAL_MEAN:-$LONG_HORIZON_EXIT_INTERVAL_MEAN}"
...
FLEET_DECISION_ACK_TIMEOUT="${PARKSIM_SUITE_FLEET_DECISION_ACK_TIMEOUT:-10.0}"
FLEET_DECISION_PERIOD="${PARKSIM_SUITE_FLEET_DECISION_PERIOD:-3.0}"
...
GATE_REQUIRE_VIDEO="${PARKSIM_SUITE_GATE_REQUIRE_VIDEO:-0}"
GATE_SKIP_VIDEO="${PARKSIM_SUITE_GATE_SKIP_VIDEO:-0}"
GATE_SKIP_MANUSCRIPT="${PARKSIM_SUITE_GATE_SKIP_MANUSCRIPT:-0}"
...
TRAFFIC_COMPLETION_STOP="${PARKSIM_SUITE_TRAFFIC_COMPLETION_STOP:-1}"
TRAFFIC_COMPLETION_GRACE_SECONDS="${PARKSIM_SUITE_TRAFFIC_COMPLETION_GRACE_SECONDS:-5}"
TRAFFIC_HORIZON_STOP="${PARKSIM_SUITE_TRAFFIC_HORIZON_STOP:-1}"
TRAFFIC_HORIZON_OVERRUN_SECONDS="${PARKSIM_SUITE_TRAFFIC_HORIZON_OVERRUN_SECONDS:-180}"
TRAFFIC_DRAIN_WALL_GRACE_SECONDS="${PARKSIM_SUITE_TRAFFIC_DRAIN_WALL_GRACE_SECONDS:-180}"
PROTECT_ACTIVE_TRAINING="${PARKSIM_SUITE_PROTECT_ACTIVE_TRAINING:-1}"
ALLOW_DURING_TRAINING="${PARKSIM_SUITE_ALLOW_DURING_TRAINING:-0}"
TRAINING_PROCESS_REGEX="${PARKSIM_SUITE_TRAINING_PROCESS_REGEX:-main_MAPPO|main_MAHAN|main_HAN|main_IPPO}"

guard_active_training() {
  if [[ "$DRY_RUN" == "1" || "$PROTECT_ACTIVE_TRAINING" != "1" || "$ALLOW_DURING_TRAINING" == "1" ]]; then
    return
  fi
  local matches
  matches="$(pgrep -af "$TRAINING_PROCESS_REGEX" || true)"
  if [[ -n "$matches" ]]; then
    echo "Refusing to start ParkSim paper suite while training is active." >&2
    echo "$matches" >&2
    echo "Wait for training to finish. Explicit override: PARKSIM_SUITE_ALLOW_DURING_TRAINING=1" >&2
    exit 75
  fi
}
...
payload = {
    ...
    "simulation_step_seconds": os.environ.get("SIMULATION_STEP", ""),
    "simulation_speedup": os.environ.get("SIMULATION_SPEEDUP", ""),
    ...
    "av_spawn_entering": int(os.environ.get("AV_SPAWN_ENTERING", "0")),
    "av_spawn_exiting": int(os.environ.get("AV_SPAWN_EXITING", "0")),
    ...
    "long_horizon_human_enter_interval_mean": os.environ.get("LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN", ""),
    "long_horizon_human_exit_interval_mean": os.environ.get("LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN", ""),
    "long_horizon_av_enter_interval_mean": os.environ.get("LONG_HORIZON_AV_ENTER_INTERVAL_MEAN", ""),
    ...
    "fleet_decision_ack_timeout": os.environ.get("FLEET_DECISION_ACK_TIMEOUT", ""),
    ...
    "gate_require_video": os.environ.get("GATE_REQUIRE_VIDEO", ""),
    "gate_skip_video": os.environ.get("GATE_SKIP_VIDEO", ""),
    "gate_skip_manuscript": os.environ.get("GATE_SKIP_MANUSCRIPT", ""),
    ...
    "protect_active_training": os.environ.get("PROTECT_ACTIVE_TRAINING", ""),
    "allow_during_training": os.environ.get("ALLOW_DURING_TRAINING", ""),
    "training_process_regex": os.environ.get("TRAINING_PROCESS_REGEX", ""),
}
...
export ROOT GIT_COMMIT GIT_BRANCH AGENTS SEEDS BACKGROUND_MODES DENSITY_CONFIGS DURATION SIMULATION_STEP SIMULATION_SPEEDUP WALL_TIMEOUT WALL_TIMEOUT_MULTIPLIER TRAFFIC_FLOW_MODE FLEET_CONTROL_MODE ENTRY_VEHICLE_AGENT_TYPE EXIT_VEHICLE_AGENT_TYPE AV_SPAWN_ENTERING AV_SPAWN_EXITING RESTORE_OBSTACLES_AS_EXIT_VEHICLES HUMAN_INTENT_HIDDEN_FRACTION MAX_CONCURRENT_BACKGROUND_VEHICLES LONG_HORIZON_ENTER_INTERVAL_MEAN LONG_HORIZON_EXIT_INTERVAL_MEAN LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN LONG_HORIZON_AV_ENTER_INTERVAL_MEAN LONG_HORIZON_AV_EXIT_INTERVAL_MEAN SPOT_INDEX QWEN_MODE QWEN_ENDPOINT QWEN_TIMEOUT FLEET_DECISION_ACK_TIMEOUT FLEET_DECISION_PERIOD QWEN_STARTUP_TIMEOUT QWEN_PORT QWEN_BENCH_MODE QWEN_MANAGED QWEN_READY CONTROLLED_EGO_BLOCKS_ENTRANCE REQUIRE_COMPLETE REFERENCE_AGENT REPLAY_ALL_DENSITIES REPORT_NAME TRC_ANALYSIS CRITICAL_STATE_ANALYSIS GATE_PROFILE GATE_STRICT GATE_REQUIRE_VIDEO GATE_SKIP_VIDEO GATE_SKIP_MANUSCRIPT RESUME DRY_RUN CONTINUE_ON_FAIL EARLY_STOP EARLY_STOP_POLL_SECONDS EARLY_STOP_GRACE_SECONDS TRAFFIC_COMPLETION_STOP TRAFFIC_COMPLETION_GRACE_SECONDS TRAFFIC_HORIZON_STOP TRAFFIC_HORIZON_OVERRUN_SECONDS TRAFFIC_DRAIN_WALL_GRACE_SECONDS PROTECT_ACTIVE_TRAINING ALLOW_DURING_TRAINING TRAINING_PROCESS_REGEX
write_suite_config

benchmark_dirs=()
for background_mode in $BACKGROUND_MODES; do
  for density in $DENSITY_CONFIGS; do
    IFS=":" read -r density_label spawn_entering spawn_exiting density_human_enter_mean density_human_exit_mean density_av_enter_mean density_av_exit_mean <<< "$density"
    if [[ -z "$density_label" || -z "$spawn_entering" || -z "$spawn_exiting" ]]; then
      echo "Invalid density config '$density'. Use label:spawn_entering:spawn_exiting[:human_enter_mean:human_exit_mean:av_enter_mean:av_exit_mean]" >&2
      exit 1
    fi
    density_human_enter_mean="${density_human_enter_mean:-$LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN}"
    density_human_exit_mean="${density_human_exit_mean:-$LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN}"
    density_av_enter_mean="${density_av_enter_mean:-$LONG_HORIZON_AV_ENTER_INTERVAL_MEAN}"
    density_av_exit_mean="${density_av_exit_mean:-$LONG_HORIZON_AV_EXIT_INTERVAL_MEAN}"
    if [[ "$background_mode" == "replay" && "$density_label" != "replay" && "$REPLAY_ALL_DENSITIES" != "1" ]]; then
      echo "skip replay density=$density_label because replay ignores random spawn density"
      continue
    fi
    if [[ "$background_mode" != "replay" && "$density_label" == "replay" ]]; then
      echo "skip background=$background_mode replay-only density profile"
      continue
    fi
    ...
    PARKSIM_BENCH_SIMULATION_SPEEDUP="$SIMULATION_SPEEDUP" \
    PARKSIM_BENCH_WALL_TIMEOUT="$WALL_TIMEOUT" \
    PARKSIM_BENCH_WALL_TIMEOUT_MULTIPLIER="$WALL_TIMEOUT_MULTIPLIER" \
    PARKSIM_BENCH_TRAFFIC_FLOW_MODE="$TRAFFIC_FLOW_MODE" \
    PARKSIM_BENCH_FLEET_CONTROL_MODE="$FLEET_CONTROL_MODE" \
    PARKSIM_BENCH_ENTRY_VEHICLE_AGENT_TYPE="$ENTRY_VEHICLE_AGENT_TYPE" \
    PARKSIM_BENCH_EXIT_VEHICLE_AGENT_TYPE="$EXIT_VEHICLE_AGENT_TYPE" \
    PARKSIM_BENCH_AV_SPAWN_ENTERING="$AV_SPAWN_ENTERING" \
    PARKSIM_BENCH_AV_SPAWN_EXITING="$AV_SPAWN_EXITING" \
    PARKSIM_BENCH_RESTORE_OBSTACLES_AS_EXIT_VEHICLES="$RESTORE_OBSTACLES_AS_EXIT_VEHICLES" \
    PARKSIM_BENCH_HUMAN_INTENT_HIDDEN_FRACTION="$HUMAN_INTENT_HIDDEN_FRACTION" \
    PARKSIM_BENCH_MAX_CONCURRENT_BACKGROUND_VEHICLES="$MAX_CONCURRENT_BACKGROUND_VEHICLES" \
    PARKSIM_BENCH_LONG_HORIZON_ENTER_INTERVAL_MEAN="$LONG_HORIZON_ENTER_INTERVAL_MEAN" \
    PARKSIM_BENCH_LONG_HORIZON_EXIT_INTERVAL_MEAN="$LONG_HORIZON_EXIT_INTERVAL_MEAN" \
    PARKSIM_BENCH_LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN="$density_human_enter_mean" \
    PARKSIM_BENCH_LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN="$density_human_exit_mean" \
    PARKSIM_BENCH_LONG_HORIZON_AV_ENTER_INTERVAL_MEAN="$density_av_enter_mean" \
    PARKSIM_BENCH_LONG_HORIZON_AV_EXIT_INTERVAL_MEAN="$density_av_exit_mean" \
    ...
    PARKSIM_BENCH_FLEET_DECISION_ACK_TIMEOUT="$FLEET_DECISION_ACK_TIMEOUT" \
    PARKSIM_BENCH_FLEET_DECISION_PERIOD="$FLEET_DECISION_PERIOD" \
    ...
    PARKSIM_BENCH_TRAFFIC_COMPLETION_STOP="$TRAFFIC_COMPLETION_STOP" \
    PARKSIM_BENCH_TRAFFIC_COMPLETION_GRACE_SECONDS="$TRAFFIC_COMPLETION_GRACE_SECONDS" \
    PARKSIM_BENCH_TRAFFIC_HORIZON_STOP="$TRAFFIC_HORIZON_STOP" \
    PARKSIM_BENCH_TRAFFIC_HORIZON_OVERRUN_SECONDS="$TRAFFIC_HORIZON_OVERRUN_SECONDS" \
    PARKSIM_BENCH_TRAFFIC_DRAIN_WALL_GRACE_SECONDS="$TRAFFIC_DRAIN_WALL_GRACE_SECONDS" \
    PARKSIM_BENCH_PROTECT_ACTIVE_TRAINING="$PROTECT_ACTIVE_TRAINING" \
    PARKSIM_BENCH_ALLOW_DURING_TRAINING="$ALLOW_DURING_TRAINING" \
    PARKSIM_BENCH_TRAINING_PROCESS_REGEX="$TRAINING_PROCESS_REGEX" \
...
if [[ "$GATE_REQUIRE_VIDEO" == "1" ]]; then
  gate_args+=(--require-video-evidence)
fi
if [[ "$GATE_SKIP_VIDEO" == "1" ]]; then
  gate_args+=(--skip-video-evidence)
fi
if [[ "$GATE_SKIP_MANUSCRIPT" == "1" ]]; then
  gate_args+=(--skip-manuscript)
fi
