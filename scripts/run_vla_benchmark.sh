...
SIMULATION_STEP="${PARKSIM_BENCH_SIMULATION_STEP:-0.1}"
SIMULATION_SPEEDUP="${PARKSIM_BENCH_SIMULATION_SPEEDUP:-1.0}"
...
LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN="${PARKSIM_BENCH_LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN:-$LONG_HORIZON_ENTER_INTERVAL_MEAN}"
LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN="${PARKSIM_BENCH_LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN:-$LONG_HORIZON_EXIT_INTERVAL_MEAN}"
LONG_HORIZON_AV_ENTER_INTERVAL_MEAN="${PARKSIM_BENCH_LONG_HORIZON_AV_ENTER_INTERVAL_MEAN:-$LONG_HORIZON_ENTER_INTERVAL_MEAN}"
LONG_HORIZON_AV_EXIT_INTERVAL_MEAN="${PARKSIM_BENCH_LONG_HORIZON_AV_EXIT_INTERVAL_MEAN:-$LONG_HORIZON_EXIT_INTERVAL_MEAN}"
...
FLEET_DECISION_PERIOD="${PARKSIM_BENCH_FLEET_DECISION_PERIOD:-3.0}"
FLEET_DECISION_ACK_TIMEOUT="${PARKSIM_BENCH_FLEET_DECISION_ACK_TIMEOUT:-10.0}"
...
PROTECT_ACTIVE_TRAINING="${PARKSIM_BENCH_PROTECT_ACTIVE_TRAINING:-1}"
ALLOW_DURING_TRAINING="${PARKSIM_BENCH_ALLOW_DURING_TRAINING:-0}"
TRAINING_PROCESS_REGEX="${PARKSIM_BENCH_TRAINING_PROCESS_REGEX:-main_MAPPO|main_MAHAN|main_HAN|main_IPPO}"
...
ensure_training_idle() {
  if [[ "$PROTECT_ACTIVE_TRAINING" != "1" || "$ALLOW_DURING_TRAINING" == "1" ]]; then
    return 0
  fi
  local matches
  matches="$(ps -eo pid=,args= | grep -E "$TRAINING_PROCESS_REGEX" | grep -v -E 'grep -E|rg ' || true)"
  if [[ -n "$matches" ]]; then
    echo "Refusing to start ParkSim benchmark while protected training is active:" >&2
    echo "$matches" >&2
    echo "Wait for training to finish; override only after explicit resource approval." >&2
    exit 75
  fi
}
write_run_config() {
...
run_episode() {
  ...
  mkdir -p "$log_dir"
  echo "running scenario=$scenario_id agent=$agent fleet_mode=$FLEET_CONTROL_MODE entry_agent=$entry_agent exit_agent=$exit_agent sim_duration=$DURATION sim_step=$SIMULATION_STEP speedup=${SIMULATION_SPEEDUP}x wall_timeout=$WALL_TIMEOUT -> $run_dir"
  cleanup_ros_processes
  ...
  PYTHONPATH="$DLP_ROOT${PYTHONPATH:+:$PYTHONPATH}" timeout "$WALL_TIMEOUT" ros2 run parksim simulator_node.py --ros-args \
    ...
    -p long_horizon_human_enter_interval_mean:="$LONG_HORIZON_HUMAN_ENTER_INTERVAL_MEAN" \
    -p long_horizon_human_exit_interval_mean:="$LONG_HORIZON_HUMAN_EXIT_INTERVAL_MEAN" \
    -p long_horizon_av_enter_interval_mean:="$LONG_HORIZON_AV_ENTER_INTERVAL_MEAN" \
    -p long_horizon_av_exit_interval_mean:="$LONG_HORIZON_AV_EXIT_INTERVAL_MEAN" \
